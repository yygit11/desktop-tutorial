...
class Pretreatment:
    ...
    def max_min_normalization(self, data):
        ...
        """
        data = deepcopy(data)
        # min = np.min(gp_data, axis=0)
        # max = np.max(gp_data, axis=0)
        # res = (gp_data - min) / (max - min)
...
#        ref = reference
#
#    # Define a new array and populate it with the corrected gp_data
...
if __name__ == "__main__":
    ...
    p = Pretreatment()
    fd = p.D1(data)
    fd = pd.DataFrame(fd,columns=y)
    fd.to_csv('../data/trainVal/MC+FD.csv',index=False)
